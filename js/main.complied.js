...
$(document).ready(function () {
  ...
  var SUITE = (function () {
    ...
    function addExtraLoad() {
      ...
      return function () {
        ex++;
        $('.extra-loads-wrapper').append('<div class="form-group"><label ' + ('for="extra' + ex + '" class="col-sm-2">Extra Load W</label><div ') + 'class="col-sm-10"><input type="text" class="form-control extra-load" ' + ('id="extra' + ex + '" placeholder="1800"></div></div>'));
      };
    }
    ...
    function calculator() {
      ...
      demands.suiteTotal = parseInt(demands.heat_ac, 10) + parseInt(demands.subTotal, 10);
      //console.log("total wattage  " + demands.suiteTotal);

      //console.log(demands.suiteTotal);
      // find answer!!
      return calMinWireAmpacity(demands.suiteTotal, entries.suite_voltage, entries.suite_phase);
    }
